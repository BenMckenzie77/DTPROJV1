...
# json lets me read the questionsV4.json file
...
# Height is 520 to make room for progress bar and topic box without squashing text boxes
...
question_label = None
counter_label  = None
topic_label    = None # Stores the label for the Topic Box
progress_canvas = None # The container for the visual bar
progress_bar   = None # The actual moving green rectangle
options_frame  = None
option_boxes   = []   
radio_buttons  = []   
selected       = None  
...
def show_question():
    global question_label, counter_label, topic_label, progress_canvas, progress_bar
    global options_frame, option_boxes, radio_buttons, selected, error_label, next_button
    ...
    # This frame creates a white container with a black border at the top
    p_frame = tk.Frame(window, bg="white", highlightthickness=2, highlightbackground="black")
    p_frame.pack(fill="x", padx=60, pady=(15, 0))
    
    # The canvas is set to white background. It is 12px tall.
    progress_canvas = tk.Canvas(p_frame, height=12, bg="white", highlightthickness=0)
    progress_canvas.pack(fill="x")
    # This is the "fill" part of the bar. It is bright green (#a8f0a8).
    progress_bar = progress_canvas.create_rectangle(0, 0, 0, 20, fill="#a8f0a8", outline="")

    # --- TOPIC BOX SECTION ---
    # This adds a small white pill-shaped box to show the sub-topic (e.g. Algebra)
    topic_frame = tk.Frame(window, bg="white", highlightthickness=2, highlightbackground="black", padx=10)
    topic_frame.pack(pady=(12, 0))
    topic_label = tk.Label(topic_frame, text="", font=(FONT_NAME, 9, "bold"), bg="white", fg="black")
    topic_label.pack()
...
def load_question():
    ...
    topic_label.config(text=f"Topic: {q['topic']}") # Updates the topic box with JSON data
    selected.set("")
    error_label.config(text="")
    
    # --- UPDATE PROGRESS BAR ---
    # This calculates the width of the green bar based on current question number
    window.update_idletasks()
    c_width = progress_canvas.winfo_width()
    bar_width = c_width * (current_q / len(questions))
